...
kind: Deployment
metadata:
  name: {{ $app.name | default (include "ofa.fullname" $root) }}
  labels:
...
spec:
  ...
  selector:
    matchLabels:
      app.kubernetes.io/name: {{ $app.name | default (include "ofa.fullname" $root) }}
      {{- include "ofa.selectorLabels" $root | nindent 6 }}
  template:
    metadata:
      labels:
        app.kubernetes.io/name: {{ $app.name | default (include "ofa.fullname" $root) }}
        app.kubernetes.io/version: "{{ $appversion }}"
        {{- include "ofa.labels" $root | nindent 8 }}
    ...
    spec:
      ...
      containers:
        - name: {{ $app.name | default (include "ofa.fullname" $root) }}
          ...
          {{- if $app.services }}
          {{- $hasPorts := false -}}
          {{- range $app.services }}{{- if .ports }}{{- $hasPorts = true }}{{- end }}{{- end }}
          {{- if $hasPorts }}
          ports:
            {{- range $app.services }}
            {{- if .ports }}
            ...
            {{- end }}
          {{- end }}
          {{- else if and $app.service $app.service.ports }}
          ports:
